...
enum
{
	...
	HEADER_CG_KEY_AGREEMENT			= 251, // _IMPROVED_PACKET_ENCRYPTION_
...
#ifdef ENABLE_MEMLEKET_SYSTEM
typedef struct SPacketGCMemleketInfo
{
	SPacketGCMemleketInfo() : bHeader (HEADER_GC_MEMLEKET) {}
	BYTE	bHeader;
	DWORD	dwVnum[3];
	DWORD	dwCount[3];
	WORD	wAffType[3];
...
{
	SPacketGCMemleketState() : bHeader(HEADER_GC_MEMLEKET_STATE), bMemleket(-1), bMemleketBonus(-1) {}
	BYTE	bHeader;
	int8_t	bMemleket;
