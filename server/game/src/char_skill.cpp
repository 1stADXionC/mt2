...
void CHARACTER::SkillLevelPacket()
{
	...
	pack.bHeader = HEADER_GC_SKILL_LEVEL_NEW;
...
bool CHARACTER::UseSkill (DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	else
	{
		...
		if (test_server)
		{
			ChatPacket (CHAT_TYPE_INFO, "[LS;968;%s;%d]", pkSk->szName, iNeededSP);
		}
...
void CHARACTER::SkillLearnWaitMoreTimeMessage (DWORD ms)
{
	...
	if (ms < 3 * 60)
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;970]");
	}
	else if (ms < 5 * 60)
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;971]");
	}
	else if (ms < 10 * 60) // 10분
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;972]");
	}
	else if (ms < 30 * 60) // 30분
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;973]");
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;974]");
	}
	else if (ms < 1 * 3600) // 1시간
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;975]");
	}
	else if (ms < 2 * 3600) // 2시간
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;976]");
	}
	else if (ms < 3 * 3600)
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;977]");
	}
	else if (ms < 6 * 3600)
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;978]");
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;979]");
	}
	else if (ms < 12 * 3600)
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;981]");
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;982]");
	}
	else if (ms < 18 * 3600)
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;983]");
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;984]");
	}
	else //if (ms < 2 * 86400)
	{
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;985]");
		ChatPacket (CHAT_TYPE_TALKING, "%s", "[LS;986]");
	}
	/*
	   str = "30%";
	   else if (ms < 3 * 86400)
