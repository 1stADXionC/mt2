...
int get_Item_Type_Value (string inputString)
{
	string arType[] =
	{
		...
		"ITEM_RING",			//32
		"ITEM_BELT",			//33 (EItemTypes 값으로 치면 33)
	};
...
int get_Item_SubType_Value (int type_value, string inputString)
{
	...
	assert (_countof (arSubType) > static_cast<unsigned int>(type_value) && "Subtype rule: Out of range!!");

	// assert 안 먹히는 듯..
	if (_countof (arSubType) <= static_cast<unsigned int>(type_value))
	{
...
bool Set_Proto_Item_Table (TItemTable* itemTable, cCsvTable& csvTable, std::map<int, const char*>& nameMap)
{
	...
	// vnum 및 vnum range 읽기.
	{
		...
		int pos = s.find ("~");
		// vnum 필드에 '~'가 없다면 패스
		if (std::string::npos == static_cast<std::string::size_type>(pos))
		{
