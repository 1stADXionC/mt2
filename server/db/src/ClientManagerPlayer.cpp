...
#ifdef ENABLE_MEMLEKET_SYSTEM
				",title_memleket,title_memleket_bonus"
#endif				  
...
#ifdef ENABLE_MEMLEKET_SYSTEM
	int tmpMemleket = -1;
	str_to_number(tmpMemleket, row[col++]);

	if (tmpMemleket < -1)
		tmpMemleket = -1;

	pkTab->bMemleket = static_cast<int8_t>(tmpMemleket);

	int tmpMemleketBonus = -1;
	str_to_number(tmpMemleketBonus, row[col++]);

	if (tmpMemleketBonus < -1)
		tmpMemleketBonus = -1;

	pkTab->bMemleketBonus = static_cast<int8_t>(tmpMemleketBonus);
#endif
