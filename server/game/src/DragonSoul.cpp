...
bool DSManager::PullOut (LPCHARACTER ch, TItemPos DestCell, LPITEM& pItem, LPITEM pExtractor)
{
	...
	// 캐릭터의 용혼석 추출 및 추가 혹은 제거. 부산물 제공.
	{
		...
		if (bSuccess)
		{
			if (pExtractor)
			{
				sprintf(buf, "dice(%d) prob(%d + %d) EXTR(VN:%d)", static_cast<int>(fDice), static_cast<int>(fProb), iBonus, pExtractor->GetVnum());
				
			}
			else
			{
				sprintf (buf, "dice(%d) prob(%d)", static_cast<int>(fDice), static_cast<int>(fProb));
			}
...
bool DSManager::DoRefineGrade (LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (ch->GetGold() < fee)
	{
		ch->ChatPacket (CHAT_TYPE_INFO, "[LS;1024]");
