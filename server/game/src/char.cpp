...
void CHARACTER::EncodeInsertPacket (LPENTITY entity)
{
	...
	if (IsPC() == true || m_bCharType == CHAR_TYPE_NPC)
	{
		...
		addPacket.bMemleket = IsPC() == true ? GetMemleket() : -1;
#endif
...
#ifdef ENABLE_MEMLEKET_SYSTEM
	tab.bMemleket = GetMemleket();
	tab.bMemleketBonus = GetMemleketBonus();
#endif
...
#ifdef ENABLE_MEMLEKET_SYSTEM
	SetMemleket (t->bMemleket);
	SetMemleketBonus(t->bMemleketBonus);
#endif
...
void CHARACTER::PointChange (BYTE type, int amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
		...
		case POINT_EXP:
		{
			...
			if (static_cast<DWORD>(amount) < 0 && exp < static_cast<DWORD>(-amount))
			{
...
void CHARACTER::PartyJoin (LPCHARACTER pLeader)
{
	...
	ChatPacket (CHAT_TYPE_INFO, "[LS;840;%s]", pLeader->GetName());
