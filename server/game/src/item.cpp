...
bool CItem::EquipTo (LPCHARACTER ch, BYTE bWearCell)
{
	...
	if (IsDragonSoul())
	{
		if (bWearCell < static_cast<int>(WEAR_MAX_NUM) || bWearCell >= static_cast<int>(WEAR_MAX_NUM) + static_cast<int>(DRAGON_SOUL_DECK_MAX_NUM) * static_cast<int>(DS_SLOT_MAX))
		{
...
EVENTFUNC (real_time_expire_event)
{
	...
	{
		if (item->GetVnum() && item->IsRideItem())
		{
...
int CItem::GiveMoreTime_Per (float fPercent)
{
	if (IsDragonSoul())
	{
		...
		DWORD given_time = fPercent * duration / 100;
...
int	CItem::GetDuration()
{
	...
	if (-1 != GetProto()->cLimitTimerBasedOnWearIndex)
	{
		return GetProto()->aLimits[(int)GetProto()->cLimitTimerBasedOnWearIndex].lValue;
	}
