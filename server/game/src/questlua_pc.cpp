	...
	int pc_give_item_from_special_item_group (lua_State* L)
	{
		...
		for (int i = 0; i < count; i++)
		{
			if (!item_gets[i])
			{
				if (dwVnums[i] == 1)
				{
					ch->ChatPacket (CHAT_TYPE_INFO, "[LS;1216;%d]", dwCounts[i]);
				}
				else if (dwVnums[i] == 2)
				{
...
teleport_area:
	...
	{
		// Return values
		//      0: The new name has been set, but logout has not occurred
		//      1: No string was passed from the script
		//      2: Failed to pass check_name
		//      3: The same name is already in use
		//      4: Success
