		...
		case QID_AUTH_LOGIN:
		{
			...
			else
			{
				...
				else
				{
					...
					std::auto_ptr<SQLMsg> _clientversionmsg(DBManager::instance().DirectQuery("SELECT version,status FROM account.version"));
					...
					{
						std::auto_ptr<SQLMsg> _gm_login(DBManager::instance().DirectQuery("SELECT mAccount FROM common.gmlist WHERE mAccount='%s'", pinfo->login));
					...
					bool bPin = false;
					std::auto_ptr<SQLMsg> _pinmsg(DBManager::instance().DirectQuery("SELECT pin FROM account.account WHERE login='%s'", pinfo->login));
