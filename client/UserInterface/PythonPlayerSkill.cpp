...
bool CPythonPlayer::__UseSkill (DWORD dwSlotIndex)
{
			...
			pSkillData->IsAutoSearchTarget())
	{
			...
			if (pSkillData->IsOnlyForAlliance())
			{
				...
				else
				{
					if (pSkillData->CanUseForMe())
					{
						...
						Tracef (" [ALERT] Although it's a skill used on allies, it is targeted at an enemy, so it is reset to self.\n");
					}
					else
					{
		...
		else
		{
			...
			if (pSkillData->CanUseForMe())
			{
				...
				Tracef (" [ALERT] No target found, using skill on the player.\n");
			}
	...
	if (pSkillData->CanChangeDirection())
	{
		...
		{
			Tracenf ("CPythonPlayer::__UseSkill(%d) - You need to set the orientation/direction based on the screen", dwSlotIndex);
		}
	}
