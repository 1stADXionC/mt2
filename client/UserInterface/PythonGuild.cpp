...
{
	m_GuildNameMap[dwID].name = c_szName;
	m_GuildNameMap[dwID].level = level;
}
...
{
	TGuildNameMap::iterator it = m_GuildNameMap.find(dwID);
	if (it == m_GuildNameMap.end())
		return false;

	*pstrGuildName = it->second.name;
	return true;
}

bool CPythonGuild::GetGuildLevel (DWORD dwID, int* pLevel)
{
	TGuildNameMap::iterator it = m_GuildNameMap.find(dwID);
	if (it == m_GuildNameMap.end())
		return false;
