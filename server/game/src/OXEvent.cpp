...
bool COXEventManager::ShowQuizList (LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket (CHAT_TYPE_INFO, "%d %s %s", m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? "[LS;858]" : "[LS;869]");
		}
	}
	...
	for (; iter != m_map_attender.end();)
	{
		...
		if (pkChar != NULL)
		{
			...
			else
			{
				...
				// 리턴값이 sizeof(chatbuf) 이상일 경?Etruncate되었다는 ?E.
