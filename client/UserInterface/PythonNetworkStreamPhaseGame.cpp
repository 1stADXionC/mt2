...
void CPythonNetworkStream::GamePhase()
{
	...
	while (ret)
	{
		...
		switch (header)
		{
			...
			case HEADER_GC_SKILL_LEVEL_OLD:
				ret = RecvSkillLevel();
				break;
			...
			case HEADER_GC_MEMLEKET:
				ret = RecvMemleketPacket();
				break;
				
			case HEADER_GC_MEMLEKET_STATE:
				ret = RecvMemleketStatePacket();
				break;
#endif	
...
#ifdef ENABLE_MEMLEKET_SYSTEM
bool CPythonNetworkStream::RecvMemleketPacket()
{
	TPacketGCMemleketInfo p;

	if (!Recv(sizeof(p.dwVnum), p.dwVnum))
		return false;

	if (!Recv(sizeof(p.dwCount), p.dwCount))
		return false;

	if (!Recv(sizeof(p.wAffType), p.wAffType))
		return false;

	if (!Recv(sizeof(p.bAffValue), p.bAffValue))
		return false;

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_ClearMemleketList", Py_BuildValue("()"));

	for (uint8_t i = 0; i < 3; ++i)
	{
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_UpdateMemleketItems", Py_BuildValue("(bkk)", i, p.dwVnum[i], p.dwCount[i]));
		PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_UpdateMemleketAffs", Py_BuildValue("(bHb)", i, p.wAffType[i], p.bAffValue[i]));
	}

	PyCallClassMemberFunc(m_apoPhaseWnd[PHASE_WINDOW_GAME], "BINARY_LoadMemleket", Py_BuildValue("()"));
	return true;
}

bool CPythonNetworkStream::RecvMemleketStatePacket()
{
	int8_t bMemleket;
	int8_t bMemleketBonus;

	if (!Recv(sizeof(bMemleket), &bMemleket))
		return false;

	if (!Recv(sizeof(bMemleketBonus), &bMemleketBonus))
		return false;

	PyCallClassMemberFunc(
		m_apoPhaseWnd[PHASE_WINDOW_GAME],
		"BINARY_SetMemleketState",
		Py_BuildValue("(ii)", bMemleket, bMemleketBonus)
	);
