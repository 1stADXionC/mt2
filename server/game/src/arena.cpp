...
{
	if (pCharFrom == NULL || pCharTo == NULL)
	{
		return false;
	}
...
EVENTFUNC (ready_to_start_event)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		pArena->SendChatPacketToObserver (CHAT_TYPE_NOTICE, "[LS;853]");
	...
	switch (info->state)
	{
		case 0:
		{
			...
			pArena->SendChatPacketToObserver (CHAT_TYPE_INFO, "[LS;856]");
		...
		case 1:
		{
			...
			pArena->SendChatPacketToObserver (CHAT_TYPE_INFO, "[LS;857]");
		...
		default:
		{
			...
			pArena->SendChatPacketToObserver (CHAT_TYPE_INFO, "[LS;859]");
...
EVENTFUNC (duel_time_out)
{
	...
	if (chA == NULL || chB == NULL)
	{
		...
		pArena->SendChatPacketToObserver (CHAT_TYPE_INFO, "[LS;853]");
	...
	else
	{
		...
		{
			case 0:
				pArena->SendChatPacketToObserver (CHAT_TYPE_NOTICE, "[LS;860]");
				pArena->SendChatPacketToObserver (CHAT_TYPE_NOTICE, "[LS;861]");
...
{
	if (pCharKiller == NULL || pCharVictim == NULL)
	{
		return false;
	}
...
bool CArena::OnDead (DWORD dwPIDA, DWORD dwPIDB)
{
	...
	{
		// 둘다 접속이 끊어졌다 ?!
		SendChatPacketToObserver (CHAT_TYPE_NOTICE, "[LS;862]");
		restart = false;
	}
	else if (pCharA == NULL && pCharB != NULL)
	{
		pCharB->ChatPacket (CHAT_TYPE_NOTICE, "[LS;863]");
		SendChatPacketToObserver (CHAT_TYPE_NOTICE, "[LS;864]");
		restart = false;
	}
	else if (pCharA != NULL && pCharB == NULL)
	{
	...
	else if (pCharA != NULL && pCharB != NULL)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			if (m_dwSetPointOfA >= m_dwSetCount)
			{
				...
				SendChatPacketToObserver (CHAT_TYPE_NOTICE, "[LS;865;%s]", pCharA->GetName());

				sys_log (0, "ARENA: Duel ended. Winner is %s(%d) Loser is %s(%d)",
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			if (m_dwSetPointOfB >= m_dwSetCount)
			{
				...
				SendChatPacketToObserver (CHAT_TYPE_NOTICE, "[LS;865;%s]", pCharB->GetName());
	...
	if (restart == false)
	{
		...
		SendChatPacketToObserver (CHAT_TYPE_INFO, "[LS;867]");
	...
	else
	{
		...
		SendChatPacketToObserver (CHAT_TYPE_INFO, "[LS;868]");
